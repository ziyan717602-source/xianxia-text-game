...
一款受 `A Dark Room` 启发的原创纯文字修仙增量叙事游戏。当前仓库已进入阶段 1C 原型：核心逻辑、地点、关系账本、状态调制事件、世界日志、localStorage 存档和基础测试已经建立，但完整第一阶段数值手感仍在调整中。
...
1. 完成阶段 1C 剩余项：前 20-30 分钟数值手感测试和首轮调参。
2. 调整“枯坐 -> 玉简 -> 吐纳 -> 日课 -> 引气入体 -> 炼气一层”的等待密度和事件触发频率。
3. 扩展普通开局路线：山居、乡塾、坊市、外门、游方。
4. 进入阶段 2：灵根五行、炼丹丹毒、突破瓶颈和宗门影子系统。
