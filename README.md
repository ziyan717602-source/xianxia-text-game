...
一款受 `A Dark Room` 启发的原创纯文字修仙增量叙事游戏。当前仓库已进入阶段 2：核心逻辑、普通出身、地点、关系账本、状态调制事件、世界日志、localStorage 存档、早期数值基准和基础测试已经建立。
...
1. 继续阶段 2：灵根五行、炼丹丹毒、突破瓶颈和宗门影子系统。
2. 让五个普通出身进入更多早期事件权重和专属日志，而不是只影响起始数值。
