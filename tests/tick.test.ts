...
describe('Tick Engine', () => {
  ...
  it('should recover stamina up to max', () => {
    let state = createInitialState();
    state.resources.stamina = 50; // set below max

    state = processTick(state, TICK_INTERVAL_MS);
    expect(state.resources.stamina).toBe(51);

    state.resources.stamina = INITIAL_MAX_STAMINA;
    state = processTick(state, TICK_INTERVAL_MS);
    expect(state.resources.stamina).toBe(INITIAL_MAX_STAMINA);
  });

  it('should process offline gains correctly', () => {
    let state = createInitialState();
    state.resources.stamina = 50;

    // Simulate 10 seconds offline
    state = processTick(state, TICK_INTERVAL_MS * 10);
    expect(state.time.tick).toBe(10);
    expect(state.resources.stamina).toBe(60);
  });
});

describe('Action System', () => {
  it('should perform "tuna" successfully with enough stamina', () => {
    const state = createInitialState();
    
    // Default stamina is max (100)
    const result = performAction(state, 'tuna');
    expect(result.success).toBe(true);
    expect(result.state.resources.stamina).toBe(state.resources.stamina - 10);
    expect(result.state.resources.qi).toBe(1);
  });

  it('should fail to perform "tuna" if stamina is low', () => {
    let state = createInitialState();
    state.resources.stamina = 5;

    const result = performAction(state, 'tuna');
    expect(result.success).toBe(false);
    expect(result.state.resources.stamina).toBe(5);
  ...
  it('should handle action risk via injected random', () => {
    ...
    // stamina is still consumed
    expect(failResult.state.resources.stamina).toBe(state.resources.stamina - 30);
    // no herbs gained
