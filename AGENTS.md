...
- 重复积累必须有升格路径：手动操作 -> 日课/批量 -> 闭关/阵法/洞府/宗门供给 -> 新决策层。已在 Action 类型中添加 `upgradeTier`（manual/daily/retreat/automated）、`upgradedFrom`（标记替代来源）、`upgradeCondition`（升格条件）三个字段。详见 `src/game/types.ts` 中的 UpgradeTier 和 UpgradeCondition 定义，以及各 action 文件中的标注。
...
**首要优先级**：验证现有 🔬 实验性草案内容的可玩性，而非继续新增内容。金丹及以上境界（元婴→化神→合体→大乘→渡劫）的突破链路、行动、事件、丹药、功法等数据结构和规则已搭建，但未经完整体验验证。在为这些高境界内容补齐可玩性验证之前，不应再新增系统或大量新内容。

**次要优先级**：PWA 离线能力、部署到静态站点、评估云存档。

不要回头重做阶段 1C/2/3 的已实现功能，除非是修 bug 或平衡回归。优先关注：1) 高境界体验链路验证（从金丹到渡劫能否实际跑通）；2) 事件池扩展（为已有系统补事件，而非新系统）；3) 离线收益批量结算和 UI 组件拆分。

各系统的验证状态（✅ 已验证 / ⚠️ 部分验证 / 🔬 实验性草案）详见 `docs/project-status.md`。
